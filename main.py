...
class AlbionApp(ctk.CTk):
    def __init__(self):
        ...
        # Logika Ikon Jendela yang Benar (Windows .ico, Linux .png)
        try:
            ...
            print(f"Gagal memuat ikon jendela: {e}")
    ...
    def setup_ui(self):
        ...
        try:
            ...
            self.my_logo = ctk.CTkImage(light_image=logo_black, dark_image=logo_white, size=(50, 55))
            self.logo_label = ctk.CTkLabel(self.sidebar_wrapper, text=" CRAFTING MASTER PRO", image=self.my_logo, compound="left", font=ctk.CTkFont(size=20, weight="bold"), text_color=("black", "white"))
            self.logo_label.pack(pady=(20, 5), padx=25, anchor="w")
        except Exception as e:
            self.logo_label = ctk.CTkLabel(self.sidebar_wrapper, text="CRAFTING MASTER PRO", font=ctk.CTkFont(size=22, weight="bold"), text_color=("black", "white"))
            self.logo_label.pack(pady=(20, 5), padx=25, anchor="w")
        ...
        self.create_field_label(f_n, "Item Name"); self.ent_name = ctk.CTkEntry(f_n, height=35, corner_radius=6, fg_color=("#e4e4e4", "#2d3135"), border_width=0); self.ent_name.insert(0, self.saved_data.get("name", "")); self.ent_name.pack(fill="x")
        self.ent_target, self.ent_out_qty = self.create_2col_input(self.sidebar, "Target Craft", "target", "", "Output / Recipe", "out_qty", "")
        
        self.create_section_label(self.sidebar, "MARKET & FEES")
        self.ent_sell, self.ent_item_val = self.create_2col_input(self.sidebar, "Sell Price", "sell_price", "", "Item Value", "item_val", "")
        ...
        self.bind_global_scroll(self.sidebar_wrapper, self._scroll_sidebar); self.bind_global_scroll(self.main_container, self._scroll_main)
    ...
    def add_static_material_row(self, name_val="", price_val="", qty_val="", is_ret=True):
        ...
        chk = ctk.CTkCheckBox(row, text="", variable=ret_var, width=20, checkbox_width=20, checkbox_height=20, fg_color="#5e6ad2", hover_color="#4854b5"); chk.pack(side="left", padx=(5, 0))
    ...
    def load_saved_materials(self):
        ...
        for i in range(5):
            if i < len(saved_mats):
                m = saved_mats[i]; self.add_static_material_row(m.get("name", ""), m.get("price", ""), m.get("qty", ""), m.get("is_ret", True))
            else: self.add_static_material_row("", "", "", True)

    def add_to_list(self):
        try:
            mats_data = []
            for mat in self.material_entries:
                p_str, q_str = mat['price'].get().strip(), mat['qty'].get().strip()
                if p_str and q_str:
                    try:
                        p_val, q_val = float(p_str), float(q_str)
                        if p_val > 0 and q_val > 0: mats_data.append({"name": mat['name'].get().strip() or f"Mat {len(mats_data)+1}", "price": p_val, "qty": q_val, "is_return": mat['is_ret'].get()})
                    except ValueError: pass
            if not mats_data: return 
            res = calculate_refining_logic(self.ent_name.get(), mats_data, float(self.ent_target.get() or 0), float(self.ent_out_qty.get() or 1), float(self.ent_sell.get() or 0), float(self.ent_item_val.get() or 0), float(self.ent_fee.get() or 0), float(self.ent_rrr.get() or 0), self.premium_var.get(), self.focus_toggle_var.get(), float(self.ent_focus_cost.get() or 0), float(self.ent_focus_pool.get() or 30000))
            self.history.append(res); self.render_expandable_card(res); self.save_current_state()
        except Exception as e: print(e)

    def render_expandable_card(self, res):
        card = ctk.CTkFrame(self.scrollable_list, corner_radius=8, fg_color=("#f4f5f7", "#1a1c1e")); card.pack(pady=5, padx=10, fill="x") 
        ...
        c_w = ctk.CTkFrame(card, fg_color="transparent"); c_w.pack(side="left", fill="x", expand=True, padx=20, pady=12) 
        h_f = ctk.CTkFrame(c_w, fg_color="transparent"); h_f.pack(fill="x")
        i_l = ctk.CTkFrame(h_f, fg_color="transparent"); i_l.pack(side="left")
        tax_info = "(Premium Tax)" if res['is_premium'] else "(Normal Tax)"
        ctk.CTkLabel(i_l, text=f"{res['name'].upper()} ➔ CRAFTABLE ITEM: {res['actual_craft']:,}  {tax_info}", font=ctk.CTkFont(size=15, weight="bold"), text_color=("black", "white")).pack(anchor="w")
        ...
        def remove_card():
            if res in self.history: self.history.remove(res); self.save_current_state()
            card.destroy()
        ...
        details_visible = tk.BooleanVar(value=False); det_f = ctk.CTkFrame(c_w, fg_color="transparent")
        def toggle():
            if details_visible.get(): det_f.pack_forget(); btn_t.configure(text="Detail ▼"); details_visible.set(False)
            else: det_f.pack(fill="x", pady=(15, 0)); btn_t.configure(text="Tutup ▲"); details_visible.set(True); self.bind_global_scroll(det_f, self._scroll_main)
        btn_t = ctk.CTkButton(h_f, text="Detail ▼", width=60, height=28, font=ctk.CTkFont(size=11), fg_color=("#e4e4e4", "#2d3135"), hover_color=("#c0c0c0", "#3e4348"), text_color=("black", "white"), command=toggle); btn_t.pack(side="right")
        
        t_dash = ctk.CTkFrame(det_f, fg_color="transparent"); t_dash.pack(fill="x", pady=(5, 10))
        header_f = ctk.CTkFrame(t_dash, fg_color="transparent"); header_f.pack(fill="x", pady=(0, 8))
        ...
        for i in range(5):
            if i < len(res['buy_list']) and res['buy_list'][i]['req_q'] > 0:
                ret_status, col_color = ("RETURN", "#3498db") if res['buy_list'][i]['is_ret'] else ("NON-RET", "#e74c3c")
            ...
            else: ctk.CTkLabel(header_f, text=f"Material {i+1}", font=ctk.CTkFont(weight="bold", size=11), width=100, text_color=("#adb5bd", "#5a5e63"), anchor="w").pack(side="left", padx=8)
        ...
        def create_card_dash_row(label_text, value_key, is_money=False, highlight=False):
            row_f = ctk.CTkFrame(t_dash, fg_color="transparent"); row_f.pack(fill="x", pady=4) 
            ...
            for i in range(5):
                ...
                if i < len(res['buy_list']) and res['buy_list'][i]['req_q'] > 0:
                    mat = res['buy_list'][i]
                    keys = {'buy': 'qty', 'need': 'req_q', 'price': 'price', 'eff': 'eff_stock', 'craft': 'craftable_m', 'return': 'sisa'}
                    val = mat[keys[value_key]]
        ...
        for label, key, money, high in [("Material To Buy", "buy", False, True), ("Material Needed", "need", False, False), ("Material Price", "price", True, False), ("Effective Stock", "eff", False, False), ("Craftable Item", "craft", False, False), ("Sisa Material", "return", False, False)]: create_card_dash_row(label, key, money, high)
        
        ctk.CTkFrame(det_f, height=2, fg_color=("#b8b8b8", "#3e4348")).pack(fill="x", pady=(15, 15)) 
        b_c = ctk.CTkFrame(det_f, fg_color="transparent"); b_c.pack(fill="x", padx=5) 
        l_c = ctk.CTkFrame(b_c, fg_color="transparent"); l_c.pack(side="left", fill="both", expand=True, padx=(0, 20)) 
        ...
        r_c = ctk.CTkFrame(b_c, fg_color="transparent"); r_c.pack(side="left", fill="both", expand=True, padx=(20, 0)) 
